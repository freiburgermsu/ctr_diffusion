...
def main():
    ...
    # --- per-module layout: pack each module as a tight, uniform sunflower
    #     (phyllotaxis) disk with the most abundant members near the centre, so that
    #     every module reads as one compact, self-contained cluster ---
    GOLDEN_ANGLE = math.pi * (3 - math.sqrt(5))
    NODE_PITCH = 3200.0   # ~spacing between adjacent nodes within a module
    module_layouts = {}
    for mi, comm in enumerate(modules):
        members = sorted(comm, key=lambda n: -mean_rel_abund.get(n, 0))   # big nodes near centre
        layout = {}
        for k, node in enumerate(members):
            rr = math.sqrt(k + 0.5) * NODE_PITCH                          # uniform areal density
            theta = k * GOLDEN_ANGLE
            layout[node] = np.array([rr * math.cos(theta), rr * math.sin(theta)])
        module_layouts[mi] = layout

    max_module_radius = max(math.sqrt(len(c)) * NODE_PITCH for c in modules)
    grid_n = int(math.ceil(math.sqrt(len(modules))))
    GRID_BUFFER = 1.15   # cell pitch as a multiple of the largest module's DIAMETER (>1 => modules never touch)
    cell = 2 * max_module_radius * GRID_BUFFER

    # visit grid cells from the centre outwards so the largest module sits in the
    # middle and the big modules don't all pile into the top row
    gc = (grid_n - 1) / 2.0
    cells = sorted(((r, c) for r in range(grid_n) for c in range(grid_n)),
                   key=lambda rc: ((rc[0] - gc) ** 2 + (rc[1] - gc) ** 2,
                                   math.atan2(rc[0] - gc, rc[1] - gc)))
    pos = {}
    for i, mi in enumerate(sorted(module_layouts, key=lambda mi: -len(modules[mi]))):
        row, col = cells[i]
        center = np.array([(col - gc) * cell, (gc - row) * cell])
        for n, local in module_layouts[mi].items():
            pos[n] = center + local

    # minor pairs/triplets + negative-only isolates on a ring hugging the grid
    if periphery:
        ring = max((np.linalg.norm(p) for p in pos.values()), default=cell) * 1.04
    ...
    ax.set_aspect("equal")   # keep the grid square and module disks circular
    # smaller markers than the spring figure: tight modules need individual nodes to be
    # visible rather than collapsing into a few giant overlapping circles
    scale = 5000 * (width / 10) * 0.35
    compressor = np.sqrt
    ...
    for mi, comm in enumerate(modules):
        ...
        verts = None
        if len(pts) >= 3:
            try:                                  # QJ joggles degenerate (collinear) inputs
                verts = pts[ConvexHull(pts, qhull_options="QJ").vertices]
            except Exception:
                verts = None
        if verts is None:                         # fall back to a circle enclosing the module
